...
#            S = ((A1 + A2*(0.42-Xh2)+A3*(0.42-Xh2)**2) * Tu**(A4+A5*(0.42-Xh2)))*np.exp(A6*xh2O)    
        ...
        #Solve ODE's
        x = odeint(vent_gas_explosion, init, t, args = (P1, R,V1, gammaE, Pf_, gammaU, mi, gas_u, S, rou, rob, gammaB, Cd, Av, f,T1, Pa))
...
def vent_gas_explosion(Y,t, P1, R,V1, gammaE, Pf_, gammaU, mi, gas_u, S, rou, rob, gammaB, Cd, Av, f, T1, Pa):
    ...
    if r<R:  #Unburnt gas venting
        ...
        if Pa/P < PcriticalInv:
            ddt_mv_mi= Cd*Av*rou/mi*(gammaU*P/rou*((1+gammaU)/2)**((1+gammaU)/(1-gammaU)))**0.5
        else:
            ddt_mv_mi= Cd*Av/mi*(2*gammaU*P*rou/(gammaU-1)*(P/Pa)**(2/gammaU)*(1-(Pa/P)**((gammaU-1)/gammaU)))**0.5
        ...
        if Pa/P < PcriticalInv:
            ddt_mv_mi= Cd*Av*rob/mi*(gammaB*P/rob*((1+gammaB)/2)**((1+gammaB)/(1-gammaB)))**0.5
        else:
            ddt_mv_mi= Cd*Av/mi*(2*gammaU*P*rob/(gammaB-1)*(P/Pa)**(2/gammaB)*(1-(Pa/P)**((gammaB-1)/gammaB)))**0.5       
