...
fuelLFP = {'CO2': 48.3, 'CO': 9.1, 'H2': 29.4, 'CH4': 5.4, 'C2H6': 0.5,
           'C3H8': 7.2}  # LFP 100 SOC Golubkov 2015 Ethylene binned to Propane
        ...
        while Xa >= 0:
            ...
            if dres['phi'] > 1:
                ...
                if dres['Tad'] > Tl:
                    ...
                    Flammable = 0
            dfOut = dfOut.append(
                {'Xf': Xf, 'Xa': Xa, 'Xi': Xi, 'phi': dres['phi'], 'Tad': dres['Tad'], 'Flammable': Flammable},
                ignore_index=True)
...
def MakePlots(dff, name):
    ...
    fgg = ff.create_ternary_contour(np.array([Xf, Xa, Xi]), Tad, pole_labels=['Fuel', 'Air', 'Inert'],
                                    interp_mode='cartesian', colorscale='Hot', showscale=True, title=name + 'Tad',
                                    showmarkers=True)
    fgg.write_image(name + "_Tad.png")
    time.sleep(1)
    fgg2 = ff.create_ternary_contour(np.array([Xf, Xa, Xi]), flm, pole_labels=['Fuel', 'Air', 'Inert'],
                                     interp_mode='cartesian', colorscale='Hot', title=name + 'Flammability',
                                     ncontours=2)
    fgg2.write_image(name + "_flm.png")
    time.sleep(1)
    fgg3 = ff.create_ternary_contour(np.array([Xf, Xa, Xi]), phi, pole_labels=['Fuel', 'Air', 'Inert'],
                                     interp_mode='cartesian', colorscale='Rainbow', ncontours=8, showscale=True,
